...
def write_track_metadata(track_info: TrackInfo, cover_art_path: Path | None = None) -> None:
    ...
    try:
        ...
        if ext in (".flac", ".ogg", ".opus"):
            ...
            if track_info.track_number is not None: mp4_audio["trkn"] = [(track_info.track_number, track_info.total_tracks or 0)]
            if track_info.disc_number is not None: mp4_audio["disk"] = [(track_info.disc_number, track_info.total_discs or 0)]
